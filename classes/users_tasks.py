...
class Task:
	...
	def new_coach_task(self, coach: Any, text: str, additional_info: Union[tuple, list, str, int, None]=None) -> NoReturn:
		"""
...
def individual_training_plan(user: Any, training_plans: list[Any]) -> NoReturn:
	...
	for plan in training_plans:
		...
		report_exs = ', '.join([i.name + (f' ({i.sets}/{i.repeats if i.repeats else "максимум"})' if i.sets else f' ({"x "+ str(i.repeats) if i.repeats else "/максимум повторений"})') for i in map(lambda ex: Exercise(ex), filter(lambda ex: plan.exercises[ex] is True, plan.exercises))])
...
def working_schedule_coach(coach: Any, user: Any, current_types:list=None):
	...
	"""
	training_types = {'personal': 'персонально', 'split': 'сплит', 'group': 'группа', 'personal_online': 'персонально онлайн', 'free': 'бесплатно (пробно)'}
